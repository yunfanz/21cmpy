...
def conv_bubbles(deltax, param_dict, Z=None, scale=None, fil=1, update=0, LE=False, visualize=False, quiet=False):
	"""
...
def run(opts, args)
	...
	ion_field = conv_bubbles(d1, param_dict, Z, scale=float(scale), fil=opts.FILTER_TYPE, update=opts.UPDATE_TYPE, 
		LE=opts.LIN, visualize=opts.VISUAL, quiet=opts.QUIET)
	if not opts.INTERACTIVE:
		np.save(opts.DIR+"xH_nohalos_z{.2f}_nf{f}_eff{.1f}_HIIfilter{i}_Tvirmin{.1f}_RHIImax{i}_{i}_{i}Mpc".format(
			Z,param_dict['nf'], param_dict['eff'], opts.FILTER_TYPE, ION_Tvir_MIN, R_BUBBLE_MAX, HII_DIM, BOX_LEN))
		return
	else:
		return ion_field
